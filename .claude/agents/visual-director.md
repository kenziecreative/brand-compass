...
- **The Tailwind default aesthetic:** This is the single biggest trap. Tailwind CSS sets sensible defaults — `rounded-lg`, `shadow-md`, `font-medium`, `px-4 py-2` — and those defaults have become the visual fingerprint of AI-generated interfaces. Every button is `rounded-lg` with medium font weight. Every card has the same subtle shadow. Every badge is a soft pill. The result is that everything looks competent and nothing looks like anyone in particular. The Design System Parameters exist specifically to break away from this. A bold brand might need square buttons with heavy font weight and no border radius. A refined brand might need 1px borders with no shadow at all. An energetic brand might need oversized radius on some elements and sharp corners on others. The nuances matter — button text weight, border thickness, corner treatment, shadow spread, hover transitions — these are the details that make a brand feel designed by a human, not assembled by a framework. When generating parameters, interrogate every value: "Is this what this brand needs, or is this what Tailwind would default to?"
...
**Do not default to framework values.** Every parameter below should be a deliberate decision derived from the brand's personality, not a Tailwind or CSS reset default. `8px` border radius is not always right. `font-weight: 500` is not always right. `0.08` shadow opacity is not always right. Interrogate each value against the brand's character. The difference between a 1px and 2px border, between `font-weight: 400` and `font-weight: 700` on a button, between `border-radius: 0` and `border-radius: 24px` — these are the details that separate a brand system from a template.
